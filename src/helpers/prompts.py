...
def read_prompt(prompt_type, request_id, prompt):
    logger.info(f"Request ID: {request_id} Searching Prompt File")
    try: 
     with open(config.system_prompt_file, 'r', encoding='utf-8') as prompt_file:
      ...
      logger.error(f"Request ID: {request_id} ERR: {err}")
      logger.error("Prompt file could not be found or is in an invalid format")
...
def reduce_prompt_tokens(prompt):
    try:
      ...
      logger.info(f"Total token saving is {(token_count - new_token_count)} | Percent is {((token_count - new_token_count) / token_count) * 100}%")
      ...
      logger.error(f"ERR: {err}")
      logger.error("Failed to reduce prompt tokens")
